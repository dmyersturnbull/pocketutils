...
class FrozeList(Sequence[T], Hashable):
    ...
    def __lt__(self, other: Sequence[T]):
        return self.__lst < self.__make_other(other)
    ...
    def __make_other(self, other: Sequence[T]) -> list[T]:
...
class FrozeDict(Mapping[K, V], Hashable):
    ...
    def __eq__(self, other: Mapping[K, V]) -> bool:
        if isinstance(self, FrozeDict):
            ...
            return dict(self.__dct) == dict(other)
        raise TypeError(f"Cannot compare to {type(other)}")
    ...
    def __make_other(self, other: Mapping[K, V]) -> dict[K, V]:
