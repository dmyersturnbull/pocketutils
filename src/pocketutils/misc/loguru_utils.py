...
T_co = TypeVar("T_co", covariant=True, bound=Logger)
Z_co = TypeVar("Z_co", covariant=True, bound=Logger)


def log_traceback(record) -> None:
    extra = record["extra"]
...
class _Defaults:
    def wrap_extended_fmt(
        ...
        *,
        ...
        def FMT(record: Mapping[str, Any]) -> str:
            ...
            f = f.replace("{{TRACEBACK}}", "{extra[traceback]}") if tb else f.replace("{{TRACEBACK}}", "")
            return f + os.linesep
    ...
    @property
    def levels_current(self: Self) -> Mapping[str, int]:
        ell = loguru.logger._core.levels
        return {e.name: e.no for e in ell.values()}

    @property
    def colors_current(self: Self) -> Mapping[str, str]:
        ell = loguru.logger._core.levels
        return {e.name: e.color for e in ell.values()}

    @property
    def icons_current(self: Self) -> Mapping[str, str]:
        ell = loguru.logger._core.levels
        return {e.name: e.icon for e in ell.values()}

    @property
    def levels_built_in(self: Self) -> Mapping[str, int]:
        return {e.name: e.no for e in _levels.values()}

    @property
    def colors_built_in(self: Self) -> Mapping[str, str]:
        return {e.name: e.color for e in _levels.values()}

    @property
    def icons_built_in(self: Self) -> Mapping[str, str]:
    ...
    @property
    def levels_extended(self: Self) -> Mapping[str, int]:
        ell = loguru.logger._core.levels
    ...
    @property
    def colors_extended(self: Self) -> Mapping[str, str]:
        ell = loguru.logger._core.levels
    ...
    @property
    def colors_red_green_safe(self: Self) -> Mapping[str, str]:
        return {
            "TRACE": "<dim>",
            "DEBUG": "<bold>",
            "INFO": "<cyan>",
    ...
    @property
    def icons_extended(self: Self) -> Mapping[str, str]:
        ell = loguru.logger._core.levels
    ...
    @property
    def fmt_simplified(self: Self) -> Callable[[Mapping[str, Any]], str]:
        return self.wrap_extended_fmt()

    @property
    def fmt_built_in(self: Self) -> Callable[[Mapping[str, Any]], str]:
        return self.wrap_plain_fmt()

    @property
    def fmt_built_in_raw(self: Self) -> str:
        return _defaults.LOGURU_FORMAT

    @property
    def fmt_extended_raw(self: Self) -> str:
        return DEFAULT_FMT_STRING

    @property
    def aliases(self: Self) -> Mapping[str, str | None]:
        return {"NONE": None, "NO": None, "OFF": None, "VERBOSE": "INFO", "QUIET": "ERROR"}
    ...
    def new_log_fn(
        ...
        def _x(__message: str, *args, **kwargs):
            logger._log(level.upper(), False, logger._options, __message, args, kwargs)
...
class _HandlerInfo:
    ...
    @property
    def to_friendly(self: Self) -> HandlerInfo:
...
class InterceptHandler(logging.Handler):
    ...
    def emit(self: Self, record) -> None:
...
class Rememberer:
    ...
    def __init__(self: Self, n_messages: int) -> None:
        self.hid: int = -1
        self._messages: deque[str] = deque(maxlen=n_messages)

    def __call__(self: Self, msg: str) -> None:
...
class LoggerWithCautionAndNotice(Logger, metaclass=abc.ABCMeta):
    ...
    def caution(self: Self, __message: str, *args, **kwargs) -> None:
        raise NotImplementedError()  # not real

    def notice(self: Self, __message: str, *args, **kwargs) -> None:
        raise NotImplementedError()  # not real


class FancyLoguru(Generic[T_co]):
    """
    Refer to module documentation instead.
    """

    def __init__(self: Self, logger: T_co = _logger) -> None:
    ...
    @staticmethod
    def new(t: type[Z_co]) -> FancyLoguru[Z_co]:
        ell = _logger.patch(log_traceback)
        logger = t(ell._core, *ell._options)
        return FancyLoguru[Z_co](logger)

    @property
    def defaults(self: Self) -> _Defaults:
        return self._defaults

    @property
    def logger(self: Self) -> T_co:
        """
    ...
    @property
    def only_path(self: Self) -> Path | None:
        try:
    ...
    @property
    def levels(self: Self) -> Mapping[str, int]:
        """
    ...
    @property
    def recent_messages(self: Self) -> Sequence[str]:
        """
    ...
    @property
    def main(self: Self) -> HandlerInfo | None:
        """
    ...
    def set_control(self: Self, enabled: bool) -> Self:
        """
    ...
    @property
    def is_control_enabled(self: Self) -> bool:
        ...
        *,
        ...
        if data is None:
            ...
            if level != data.no:  # loguru doesn't check whether they're eq; it just errors
                msg = f"Cannot set level={level}!={data.no} for {name}"
    ...
    def add_log_methods(self: Self, *, replace: bool = True) -> Self:
        levels = [level.lower() for level in self.levels]
        for level in levels:
    ...
    def enable(self: Self, *names: str) -> Self:
        """
    ...
    def disable(self: Self, *names: str) -> Self:
        """
    ...
    def intercept_std(self: Self, *, warnings: bool = True) -> Self:
        """
    ...
    def remember(self: Self, *, n_messages: int = 100) -> Self:
        """
    ...
    def remove_paths(self: Self) -> Self:
        """
        ...
        if not self._control_enabled:
            return self
        for p in dict(self._paths):
            self.remove_path(p)
        return self

    def remove_path(self: Self, path: Path) -> Self:
        """
    ...
    def from_cli(
        ...
        if main is not None and main is not _SENTINEL:
            ...
            if main not in self._defaults.levels_extended:
                ...
                msg = f"{main.lower()} not a permitted log level (allowed: {_permitted}"
                raise XValueError(
                    msg,
                    value=main,
                )
    ...
    def rewire_streams_to_utf8(self: Self) -> Self:
        """
    ...
    @classmethod
    def guess_file_sink_info(cls: type[Self], path: str | Path) -> LogSinkInfo:
        path = Path(path)
        ...
        if not [base.endswith(s) for s in [".json", ".log", ".txt"]]:
            msg = f"Log filename {path.name} is not .json, .log, .txt, or a compressed variant"
            raise XValueError(
                msg,
                value=path.name,
            )
    ...
    def _get_info(self: Self, level: str, fmt: str, filter_) -> tuple[str, int, Formatter, Any]:
        if level is _SENTINEL and self._main is None:
            level = self._defaults.level
        elif level is _SENTINEL:
            level = self._main.level
        if filter_ is _SENTINEL and self._main is None:
            filter_ = None
        elif filter_ is _SENTINEL:
            filter_ = self._main.filter
