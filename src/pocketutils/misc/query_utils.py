...
class QueryExecutor:
    ...
    @property
    def last_time_taken(self: Self) -> TimeTaken:
    ...
    def __call__(
        ...
        data = content.decode(errors=errors) if encoding is None else content.decode(encoding=encoding, errors=errors)
        now_ = time.monotonic()
...
class QueryMixin:
    @property
    def executor(self: Self) -> QueryExecutor:
        raise NotImplementedError()

    def _query(self: Self, url: str, *, sink: Callable[[str], Any] = logger.debug) -> str:
